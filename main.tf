...
resource "aws_instance" "this" {
  ...
  metadata_options {
    ...
    http_tokens            = try(each.value.metadata_options.http_tokens, "optional")
    instance_metadata_tags = try(each.value.metadata_options.instance_metadata_tags, "disabled")
  }
  tags = merge(
    {
